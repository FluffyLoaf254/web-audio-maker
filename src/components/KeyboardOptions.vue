...
import { type NodeData, type Node, type ComplexDataItem, type BeatTransition, type Note } from '../types';
...
const beats = computed(() => node.value?.beats ?? 0);

const convertPixelsToRem = (pixels: number): number => {
  return pixels / parseFloat(getComputedStyle(document.documentElement).fontSize);
};

const addValue = (value: Note) => {
  meta.value.notes.push(value);
};

const removeValue = (value: Partial<Note>) => {
  meta.value.notes = meta.value.notes.filter(note => {
    for (let property in value) {
      if (note[property as keyof Note] != value[property as keyof Note]) {
        return true;
      }
    }
...
const hasValue = (value: Partial<Note>) => {
...
watch(meta, (value: Meta) => {
  let frequency = data.value.frequency as ComplexDataItem;
  frequency.array = value.notes.map((item: Note) => ({
    beat: item.beat,
    index: item.index,
...
const viewBox = computed(() => {
  return '0 0 ' + beats.value * 2 + ' ' + values.value * 2;
});
