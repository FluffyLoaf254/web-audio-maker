...
<template>
  <div class="flex flex-col bg-gray-50 w-64 dark:bg-slate-600">
    <div class="w-64 p-2">
      <form-input name="node-types-search" v-model="input" class="w-full" placeholder="Search" />
    </div>
    <div class="w-64 overflow-y-auto flex flex-col divide-y divide-gray-300 dark:divide-black max-h-full">
      <button type="button" v-for="nodeType in foundTypes" :key="nodeType.type" :disabled="numberOfType(nodeType.type) >= nodeType.max" @click="$emit('add', nodeType)" :style="{ 'border-left-color': getCategoryOf(nodeType)?.color ?? 'black' }" class="cursor-pointer text-left p-3 bg-white disabled:bg-gray-50 disabled:cursor-not-allowed disabled:text-gray-500 hover:bg-gray-100 active:bg-gray-200 border-l-8 flex justify-between items-center focus:outline-none focus:ring focus:ring-inset focus:ring-blue-500 dark:bg-slate-700 dark:hover:bg-slate-500 dark:active:bg-slate-400 dark:text-gray-200">
        {{ nodeType.name }}
