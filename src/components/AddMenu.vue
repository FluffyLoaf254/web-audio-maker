<template>
  <div class="flex flex-col bg-gray-50 w-64">
    ...
    <div class="overflow-y-auto flex flex-col divide-y max-h-full">
      <button type="button" v-for="node in foundNodes" :key="node.type" :disabled="$store.getters.numberOfType(node.type) >= node.max" @click="$emit('add', node)" :style="{ 'border-color': getCategoryOf(node).color }" class="cursor-pointer text-left p-3 bg-white disabled:bg-gray-50 disabled:cursor-not-allowed disabled:text-gray-500 hover:bg-gray-100 active:bg-gray-200 focus:outline-none focus:ring focus:ring-blue-500 border-l-8">
        {{ node.name }}
      </button>
    </div>
  </div>
